...
$(document).ready(function(){
                  ...
                  setupWebViewJavascriptBridge(function(bridge) {
                                               BRIDGE=bridge;BRIDGE.init(function(message,responseCallback){});
                                               });
                  });
...
//    $(".thread_details .P_share div span:eq(2)").html(JSON.Variables.thread.replies);
    ...
    if(!textIsNull(JSON.Variables.special_debate)){
        var timestamp = new Date().getTime().toString();
        timestamp = timestamp.substr(0,10);
        var special_debate = JSON.Variables.special_debate;
        $(".acTips span:eq(1)").html(special_debate.endtime);
        $(".acTips span:eq(2)").html(special_debate.umpire);
        if(special_debate.dbendtime < timestamp) {
            if (special_debate.winner != 0) {
                if(special_debate.winner == 1) {
                    $(".acTips span:eq(0)").html("正方获胜，");
                } else if (special_debate.winner == 2) {
                    $(".acTips span:eq(0)").html("反方获胜，");
                }
                $(".endDebateResult").removeAttr("hidden");
                $(".endDebateResult p:eq(0)").html("裁判观点：" + special_debate.umpirepoint);
                $(".endDebateResult p:eq(1)").html("最佳辩手：" + special_debate.bestdebaterurl);
            }
            else {
                $(".acTips span:eq(0)").html("辩论已于");
            }
        }
        
        var percent1 = 0;
        var percent2 = 0;
        if(special_debate.affirmvotes == 0 && special_debate.negavotes == 0) {
            $(".thread_details .detail .ratiobox .sqRatio").css("width","50%");
        } else {
            if (parseInt(special_debate.affirmvotes) == 0){
                percent1 = 0;
                percent2 = 1;
            } else if (parseInt(special_debate.negavotes) == 0){
                percent1 = 1;
                percent2 = 0;
            } else {
                percent1 = parseInt(special_debate.affirmvotes) / (parseInt(special_debate.affirmvotes) + parseInt(special_debate.negavotes));
                percent2 = 1 - percent1;
            }
            $(".thread_details .detail .ratiobox .sqRatio").css("width",percent1 * 100 + "%");
        }
        
        if (percent1 * 100 > 4){
            $(".thread_details .detail .ratiobox .vs").css("left",(percent1 * 100 - 5) + "%");
        } else if (percent1 == 0 && percent2 == 0){
            $(".thread_details .detail .ratiobox .vs").css("left","47%");
        } else {
            $(".thread_details .detail .ratiobox .vs").css("left","0%");
        }
        $(".thread_details .detail .viewPercent .sq span").html(parseInt(percent1 * 100) + "%");
        $(".thread_details .detail .viewPercent .sqr span").html(parseInt(percent2 * 100) + "%");
        
        $(".thread_details .viewWrapper p.viewNum:eq(0)").html("(" + special_debate.affirmvotes+")");
        $(".thread_details .viewWrapper p.viewNum:eq(1)").html("("+special_debate.negavotes+")");
        $(".thread_details .detail p.viewsCon:eq(0)").html(special_debate.affirmpoint);
        $(".thread_details .detail p.viewsCon:eq(1)").html(special_debate.negapoint);
        $(".thread_details .detail div.joinNum:eq(0) span").html("正方辩手：" + special_debate.affirmdebaters);
        $(".thread_details .detail div.joinNum:eq(1) span").html("反方辩手：" + special_debate.negadebaters);
        $(".thread_details .detail span.sqBtn").click(function(){supportDebate("1")});
        $(".thread_details .detail span.conBtn").click(function(){supportDebate("2")});
        $(".thread_details .detail div.joinNum:eq(0) a").click(function(){joinDebate("1")});
        $(".thread_details .detail div.joinNum:eq(1) a").click(function(){joinDebate("2")});
        if(!textIsNull(special_debate.affirmavatars)) {
            $(".thread_details .joinInfo:eq(0)").removeAttr("hidden");
            for (var p in special_debate.affirmavatars) {
                var affirmItem = special_debate.affirmavatars[p];
                $(".thread_details .detail div.joinInfo:eq(0)").append("<dl authorid=" + affirmItem.authorid +"><dt>"+ affirmItem.avatar + "</dt><dd>" + affirmItem.author +"</dd></dl>");
            }
        }
        if(!textIsNull(special_debate.negaavatars)) {
            $(".thread_details .joinInfo:eq(1)").removeAttr("hidden");
            for (var p in special_debate.negaavatars) {
                var negaItem = special_debate.negaavatars[p];
                $(".thread_details .detail div.joinInfo:eq(1)").append("<dl authorid=" + negaItem.authorid +"><dt>"+ negaItem.avatar + "</dt><dd>" + negaItem.author +"</dd></dl>");
            }
        }
        
        var dlDom = $(".joinInfo dl");
        dlDom.each(function(index, item) {
                   $(this).click(index, function() {
                                 userInfo($(this).attr("authorid"));
                                 })
                   })
        
        
        
        //        if (special_debate.umpire == JSON.Variables.member_username && timestamp < special_debate.dbendtime) {
        //            $(".thread_details .detail").append("<a class='joinBtn'>结束此次辩论</a>");
        //            $(".thread_details .joinBtn").click(function(){endDetate()});
        //        }
        
        var positionItem = $(".positionSec a");
        positionItem.each(function(index, item) {
                          $(this).click(index, function() {
                                        
                                        positionItem.each(function(newIndex, item) {
                                                          if(newIndex == index) {
                                                          $(this).attr("class","positionItem itemActive");
                                                          } else {
                                                          $(this).attr("class","positionItem");
                                                          }
                                                          })
...
//                                        alert(ItemIndex,index);
    ...
    if(!textIsNull(JSON.Variables.postlist)){
        ...
        $("div#thread_content img").click(is2GOr3GLoadImgs?function(){threadThumbsClick($(this).attr("src"));}:null)
...
var onLoadReply=function(threadJSON,isAppend){
    ...
    if(!textIsNull(postlist)){
        for(var i=isAppend?0:1;i<postlist.length;i++){
            ...
            if(postlistitem.stand == 1) {
                li.find("div.otherPostItem .topInfo").append("<span class='debateInfo pull-right'>" + "正方" +"<span class='supinfo'>" + "支持"+ postlistitem.voters + "</span></span>");
            } else if (postlistitem.stand == 2){
                li.find("div.otherPostItem .topInfo").append("<span class='debateInfo conDebate pull-right'>" + "反方" + "<span class='supinfo'>" + "支持"+ postlistitem.voters + "</span></span>");
            } else {
                li.find("div.otherPostItem .topInfo").append("<span class='debateInfo middleDebate pull-right'>" + "中立" + "</span>");
            }
            li.find("div.otherPostItem .topInfo span:eq(1)").click((postlistitem.stand!=0)?function(){supportReplyDebate($(this).parents("li").attr("pid"));}:null);
...
function supportDebate(code) {
    if(BRIDGE!==undefined){
        BRIDGE.callHandler("supportDebate",code,null);
    }
}

/**加入辩论 0为正方 1为反方*/
function joinDebate(code) {
    if(BRIDGE!==undefined){
        BRIDGE.callHandler("joinDebate",code,null);
    }
}
...
function endDetate() {
    if(BRIDGE !==undefined) {
        BRIDGE.callHandler("endDebate",null,null);
    }
}
...
function changeVideoCss()
{
    var arr = document.getElementsByTagName('iframe');
    var num = arr.length;
    for(var i=0;i<num;++i)
    {
        if(arr[i].className == "video_iframe")
        {
            arr[i].className = "videoframe";
        }
    }
}
